...
session_start();
$idC = connexion();
$reqSelection = "select count(*) from utilisateur where Email_Utilisateur ='".$_POST['identifiant']."' and MotDePasse_Utilisateur = '".$_POST['password']."';";
    ...
    if (isset($_POST['login']) /*&& $idC!=false*/) { // le bouton inserer a a été cliqué
            ...
            foreach ($lignesResult as $uneLigne){
                if ($uneLigne[0] == 1) {
                    ...
                    header("refresh:1;url=../espace_client.php");
                }
                else {
